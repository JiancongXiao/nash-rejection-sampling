...
def validate_run_step_records(
    records: Iterable[dict], *, expected_steps: int
) -> list[dict]:
    """Validate the common artifact contract for a non-smoke Main run."""
...
def write_run_manifest(
    ...
    *,
    ...
    if changed <= 0 or update_norm <= 0:
        raise ValueError("checkpoint did not change from the base model")
    totals = {
        field: sum(float(record[field]) for record in values)
        for field in ("preference_model_calls", "generated_tokens", "gpu_hours")
    }
    manifest = {
        "suite": MAIN_SUITE,
        "run_kind": run_kind,
