...
MODEL_CACHE="/scratch/jiancongxiao/cache/huggingface/hub/models--Qwen--Qwen2.5-7B-Instruct"
SUBMISSION_ROOT="/scratch/jiancongxiao/results/main-native-7b-lora-8192-submissions"
...
if [[ "$(wc -l < "$PROMPTS" | tr -d '[:space:]')" != "8192" ]]; then
  echo "Frozen 7B prompt file must contain exactly 8192 rows" >&2
  exit 3
fi
...
if compgen -G "$MODEL_CACHE/snapshots/*/config.json" >/dev/null; then
  ...
  CACHE_JOB="$(qsub cluster/hopper/main/cache_qwen2p5_7b.pbs)"
fi
...
for method in "${METHODS[@]}"; do
  ...
  job="$(qsub "${dependency[@]}" \
    ...
    cluster/hopper/main/main_7b_lora_8192.pbs)"
